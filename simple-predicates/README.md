...
```ts
import { isString, type SimplePredicate } from "z-simple-predicates";
...
`isObject`, `isArray`, `isFunction`, `isError`
...
- **Broken values are rejected by their own type's predicate.**
  `isNumber(NaN)` is `false`: a value that is unusable as a number is not
  one. Use `Number.isNaN` directly when the question really is "is this
  `NaN`".
- **`isFunction` is `typeof value === 'function'`**, which is also true of
...
- **Built-in identification** — `isDate`, `isRegExp`, `isPromise`, `isMap`, `isSet`, `isPlainObject`, `isThenable`. Answering
...
Composition, configuration, formats, and reporting belong in a
higher-level package that can depend on this one. Built-in identification
is not a layer above but a heavier one alongside, and follows separately.
